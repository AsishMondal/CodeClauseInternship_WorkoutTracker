...
static LOCAL_STORAGE_DATE_KEY = "workout-tracker-entries";
        ...
        return `
            ...
            <td>
                <select class="tracker__workout">
                    ...
                    <option value="outdoor-cycling">Outdoor_Cycling</option>
                    <option value="indoor-cycling">Indoor_Cycling</option>
    ...
    loadEntries() {
        this.entries = JSON.parse(localStorage.getItem(WorkoutTracker.LOCAL_STORAGE_DATE_KEY) || "[]");
    }

    saveEntries() {
        localStorage.setItem(WorkoutTracker.LOCAL_STORAGE_DATE_KEY, JSON.stringify(this.entries));
    }
    ...
    updateView(){
        ...
        const addRow = data => { 
            ...
            row.querySelector(".tracker__date").vale = data.date;
            row.querySelector(".tracker__workout").vale = data.workout;
            row.querySelector(".tracker__duration").vale = data.duration;

            row.querySelector(".tracker__date").addEventListener("chanhe", ({ target }) =>{
                data.date = target.value;
                this.saveEntries();
            });

            row.querySelector(".tracker__workout").addEventListener("chanhe", ({ target }) =>{
                data.workout = target.value;
                this.saveEntries();
            });

            row.querySelector(".tracker__duration").addEventListener("chanhe", ({ target }) =>{
